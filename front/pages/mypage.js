...
export default function Mypage() {
    ...
    const imageUrl = user?.ufile ? '${process.env.NEXT_PUBLIC_BASE_URL}/${user.ufile}' : undefined;
    ...
    //view
    return (<Card title="마이페이지" style={{ maxWith: 800, margin: "20px auto", borderRadius: "30px", background: "#fafafa" }}>
        <Tabs defaultActiveKey="profile">
            {/* 내 정보 탭 */}
            <TabPane tab="내정보" key="profile">
                <div style={{ display: "flex", alignTems: "center", marginBottom: 20 }}>
                    <Avatar src={imageUrl} size={64}>
                        {user.nickname?.[0]}
                    </Avatar>
                    <Descriptions bordered column={1} size="middle" style={{ marginLeft: 20, borderRadius: "10px", overflow: "hidden" }}>
                ...
                <Form
                    ...
                    layout="inline"
                    style={{ marginBottom: 20, borderRadius: "5px" }}
                >
                    <Form.Item
                        name="nickname"
                        rules={[{ required: true, message: "새 닉네임을 입력하세요" }]}
                    >
                        <Input placeholder="새 닉네임" style={{ borderRadius: "5px" }} />
                    </Form.Item>
                    <Button type="primary" htmlType="submit" style={{ borderRadius: "5px" }}>닉네임 변경</Button>
                </Form>

                {/* 프로필 이미지  수정*/}
                <Form layout="inline" style={{ marginBottom: 20, borderRadius: "5px" }}>
                    <Form.Item>
                        <Upload
                            beforeUpload={() => false}
                            fileList={fileList}
                            ...
                            maxCount={1}
                        >
                            <Button icon={<UploadOutlined />} style={{ borderRadius: "5px" }}>이미지 선택</Button>
                        </Upload>
                    </Form.Item>
                    <Button
                        type="primary"
            ...
            {/* 팔로잉 탭 */}
            <TabPane tab={`팔로링 ${followingsList.length}`} key="followings">
                <List loading={followLoading}
                    ...
                    renderItem={(item) => (
                        <List.Item actions={[
                            <Button key="unfollow"
                                style={{ borderRadius: "5px" }}
                                onClick={() => dispatch(unfollowRequest({ followeeId }))}
                            >
                                언팔로우
                            </Button>
                        ]}>
