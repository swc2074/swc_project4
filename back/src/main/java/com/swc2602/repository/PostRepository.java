package com.swc2602.repository;
...
public interface PostRepository extends JpaRepository<Post, Long> { // Entity , PK ★
	...
	@Query(value = "SELECT * FROM ( " +
			"SELECT p.*, ROWNUM AS rnum " +
			" FROM ( " +
			"   SELECT po.ID, po.CONTENT, po.CREATED_AT, po.DELETED, po.UPDATED_AT, po.APP_USER_ID, po.LINK " +
			"   FROM POSTS po " +
			...
			") " +
			"WHERE rnum BETWEEN :start AND :end", nativeQuery = true)
	List<Post> findMyPostsAndRetweetsWithPaging(@Param("userId") Long userId, @Param("start") int start,
			@Param("end") int end);

	/////////////////////////////////
	// List<Post> findPostsWithPaging( @Param("start") int start, @Param("end") int
	///////////////////////////////// end );
}
