...
describe('postSaga', () => {
  ...
  // --- 글 생성 ---         createPostRequest, createPostSuccess, createPostFailure,
   it('createPostSuccess', async () => {
    ...
    await runSaga({ dispatch: (a) => dispatched.push(a) }  , createPost , action )
          .toPromise();
  ...
  it('updatePostSuccess', async () => {
    ...
    await runSaga({ dispatch: (a) => dispatched.push(a) }  , updatePost , action )
          .toPromise();
  ...
  it('deletePostSuccess', async () => {
    ...
    await runSaga({ dispatch: (a) => dispatched.push(a) }  , deletePost , action )
          .toPromise();

    expect(dispatched).toContainEqual(deletePostSuccess( 1)); // reducer에 넘길 값
  });
}); 
////////////// npm run test
