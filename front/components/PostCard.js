                ...
                <>
                    ...
                    {post?.link && (
                        <div style={{ marginTop: "8px" }}>
                            <span style={{ fontWeight: "bold", marginRight: "5px" }}>구매사이트:</span>
                            {post.link.split(",").map((link, idx) => {
                                const cleanLink = link.trim();
                                if (!cleanLink) return null;
                                // 간단한 URL 보정 (http 없으면 추가)
                                const href = cleanLink.startsWith("http") ? cleanLink : `http://${cleanLink}`;
                                return (
                                    <a
                                        key={`${cleanLink}-${idx}`}
                                        href={href}
