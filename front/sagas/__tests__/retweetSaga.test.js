...
describe('postSaga', () => {
  ...
  it('addRetweetSuccess', async () => {
    ...
    const  action = addRetweetRequest({originalPostId: 1});   // react - view
    await runSaga({ dispatch: (a) => dispatched.push(a) }  , addRetweet , action ).toPromise();

    expect(dispatched).toContainEqual(addRetweetSuccess( {originalPostId: 1, retweetCount: 5}));
  });
...
// --- 리트윗 있는지 여부  ---hasRetweetedRequest, hasRetweetedSuccess, hasRetweetedFailure,
  ...
  // --- 내 리트윗  ---fetchMyRetweetsRequest, fetchMyRetweetsSuccess, fetchMyRetweetsFailure,
